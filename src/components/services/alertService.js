...
const alertService = {
     ...
     */
    async getAlertHistory(page = 1, limit = 10) {
        try {
     ...
     */
    formatDetectionToAlertPayload(detection, recipientEmail, customMessage) {
        return {
     ...
     */
    generateDefaultAlertMessage(detection) {
        const missingItems = [];
        if (!detection.hasHelmet)
            missingItems.push('casque');
        if (!detection.hasVest)
            missingItems.push('gilet');
        if (!detection.hasGloves)
            missingItems.push('gants');
        return `Alerte sécurité : Équipement manquant (${missingItems.join(', ')}). Veuillez régulariser cette situation immédiatement.`;
    },
};
// src/services/alertService.ts
// Export nommé (recommandé)
export const getAlertHistory = async (page = 1, limit = 10) => {
    try {
        const response = await apiClient.get('/alerts/history', {
            params: { page, limit },
        });
